...
def test_decode_noising_follows_flow_matching_interpolation_convention(monkeypatch):
    ...
    expected_noised = LatentCreator.add_noise_by_interpolation(clean=clean_latent, noise=expected_eps, sigma=degrade_sigma)

    assert mx.allclose(captured["lq_latent"].astype(mx.float32), expected_noised.astype(mx.bfloat16).astype(mx.float32), atol=1e-2)
