...
from mflux.models.z_image.cli import z_image_generate, z_image_turbo_generate, z_image_turbo_generate_controlnet
...
@pytest.mark.fast
def test_z_image_turbo_controlnet_cli_warns(monkeypatch):
    messages = _run_main(
        monkeypatch,
        z_image_turbo_generate_controlnet,
        "ZImageTurboControlnet",
        ["--prompt", "x", "--control", "pose:pose.png", "--negative-prompt", "y"],
    )
    assert any("--negative-prompt is ignored" in m for m in messages)


@pytest.mark.fast
def test_boogu_cli_warns_on_both(monkeypatch):
    messages = _run_main(
        monkeypatch,
        boogu_image_generate,
        "BooguImage",
        ["--prompt", "x", "--guidance", "3.0", "--negative-prompt", "y"],
    )
    assert any("--guidance is ignored" in m for m in messages)
