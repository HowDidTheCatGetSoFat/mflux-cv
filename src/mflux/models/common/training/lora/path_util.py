...
def expand_module_paths_from_targets(targets: list[LoraTargetSpec]) -> list[tuple[str, int, float | None, bool]]:
    expanded: list[tuple[str, int, float | None, bool]] = []
    for t in targets:
        expanded.extend((p, t.rank, t.alpha, t.use_dora) for p in expand_module_paths(t))
    return expanded
