...
class LoRALoader:
    ...
    def _apply_single_lora(
        ...
        *,
        ...
        if applied_count == 0:
            # The one case where the user has no idea what went wrong must not be the one
            # case that says nothing: show how the file's keys end against what the
            ...
            raise ValueError(
                ...
                f"without a trailing .weight) under its module paths; anything else, e.g. full "
                f"weight diffs (.diff), has no LoRA mechanism to land on."
            )
    ...
    def _match_pattern(weight_key: str, pattern: str) -> int | None:
        ...
        # its transformer. prefix; a field-patched krea2 was the report that surfaced
        # the gap). Exact equality on the derived string cannot over-match: both
        # spellings of a pattern name the same target module.
